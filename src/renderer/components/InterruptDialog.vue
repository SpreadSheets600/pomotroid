  ...
  <transition name="fade">
    <div v-if="show" class="InterruptDialog-overlay" @click="closeDialog">
      ...
      <div class="InterruptDialog-header">
        <h3>Focus Session Interrupted</h3>
      </div>

      <div class="InterruptDialog-content">
        <p class="InterruptDialog-message">Select a reason (optional):</p>
...
export default {
  ...
  methods: {
    ...
    submitReason() {
      const reason = this.customReason.trim() || this.selectedReason || 'No reason provided'
    ...
    skipReason() {
      this.$store.dispatch('submitInterruptReason', 'No reason provided')
      this.closeDialogOnly()
      this.resetForm()
    },

    closeDialog() {
      this.$store.dispatch('submitInterruptReason', 'Dismissed by user')
      this.closeDialogOnly()
      this.resetForm()
    },
